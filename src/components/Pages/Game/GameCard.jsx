    ...
    <article className="w-full max-w-[350px] bg-white  rounded-xl shadow-md overflow-hidden transform transition hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          <div className="text-sm font-medium text-secondary bg-yellow-300 rounded-xl px-3 py-1">{game?.category}</div>
        </div>

        <div className="mt-4 flex gap-3">

          <Link to={`/game-details/${game?.id}`} className="flex-1 inline-flex items-center justify-center rounded-md px-3 py-2 text-sm font-medium bg-secondary text-white hover:bg-indigo-700 transition"  >Details</Link>
          ...
          <button
            ...
            <a 
            target="_blank"
            href={game.downloadLink}>Download</a>
          </button>
        </div>
      </div>
    </article>
